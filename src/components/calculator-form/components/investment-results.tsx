...
export const InvestmentResults = () => {
	const { investmentResults } = useAppSelector((state) => state.targetsCalculator);
	const { profit, stopLoss } = investmentResults;
			...
			<CardBody>
				<div className="grid xs:grid-cols-2 md:grid-cols-4 xs:gap-3 md:gap-1">
					...
					<div className="flex flex-col gap-unit-xs items-start">
						...
						<div className="flex gap-4 xs:text-sm md:text-md">
							<ProfitLossResult amount={profit.totalExitAmount} currencyCode={profit.currencyCode} isLoss={profit.isLoss} />
						</div>
					</div>
					...
					<div className="flex flex-col gap-unit-xs items-start">
						...
						<div className="flex gap-4 xs:text-sm md:text-md">
							<ProfitLossResult amount={stopLoss.totalExitAmount} currencyCode={stopLoss.currencyCode} isLoss={stopLoss.isLoss} />
						</div>
					</div>
				</div>
			</CardBody>
